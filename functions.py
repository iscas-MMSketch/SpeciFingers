...
class Dataset_CRNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform):
        ...
        for i in self.frames:
            image = Image.open(os.path.join(path, selected_folder, 'image_{:05d}.jpg'.format(i)))
...
def CRNN_final_prediction_R(model, device, loader):
    cnn_encoder, rnn_decoder = model
    cnn_encoder.eval()
    rnn_decoder.eval()
...
class AlexCNNEncoder(nn.Module):
    def __init__(self, fc_hidden1=512, fc_hidden2=512, drop_p=0.3, CNN_embed_dim=300):
        ...
        # self.fc1 = nn.Linear(alexnet.classifier.in_features, fc_hidden1)
